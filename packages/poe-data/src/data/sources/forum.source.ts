...
export interface ForumSource {
  ...
  fetchBuildList(league: string, _class: string): Promise<AdapterResult<ForumThread[]>>;
}

export function createForumSource(httpClient: HttpClient): ForumSource {
  return {
    name: 'GGGForum',
    baseUrl: FORUM_BASE,

    async fetchBuildList(league: string, _class: string): Promise<AdapterResult<ForumThread[]>> {
      try {
        ...
        return { ok: false, error: `Forum fetch failed: ${err instanceof Error ? err.message : String(err)}` };
      }
    },
  };
}
